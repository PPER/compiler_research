...
def crossover(parents, offspring_size):
    ...
    for k in range(offspring_size):
        ...
        parent2_idx = (k+1+index)%(len(parents))
        if (k+1+index)%(len(parents)) == 0:
            index = index+1
...
def mutation(offspring_crossover):
    ...
    random_value = numpy.random.randint(1,opt2_len+opt3_len)
...
dict_sum = {'generation': [str(0)+" SUMMARY"], 'space': [new_population[small_id[0]]],'flags': [flags[small_id[0]]], 'time': [min(fitness)]}    
df_sum = pd.DataFrame(dict_sum) 
frames = [df,df_new,df_sum]
df = pd.concat(frames)
...
for generation in range(1, num_generations):
    ...
    small_id = smallest_N_index(1,fitness)
    dict_sum = {'generation': [str(generation)+" SUMMARY"], 'space': [new_population[small_id[0]]],'flags': [flags[small_id[0]]], 'time': [min(fitness)]}    
    df_sum = pd.DataFrame(dict_sum) 
    frames = [df,df_new,df_sum]
    df = pd.concat(frames)
